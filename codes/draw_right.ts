...
ifThen(other.bulletPos().notEqual(-1), () => {
    const pos = other.bulletPos().add(26)
    ifThen(pos.greaterThan(176), () => {
        draw.poly(pos.sub(176), 88, 16, 8, 0)
    })
})

draw.color(32, 96, 255, 255)
draw.poly(176, 88, 6, 16, 30)

ifThen(data.bulletPos().notEqual(-1), () => {
    const pos = operation("sub", 300, data.bulletPos()).get().add(26)
    ifThen(pos.greaterThan(176), () => {
        draw.poly(pos.sub(176), 88, 16, 8, 0)
    })
})

const laserRight = variable().set(160)

ifThen(data.shielded().notEqual(0), () => {
    draw.stroke(2)
    draw.line(149, 73, 149, 103)
    draw.line(149, 73, 144, 88)
    draw.line(149, 103, 144, 88)
    laserRight.set(144)
})
...
ifThen(data.summon().notEqual(-1), () => {
    ...
    draw.image(88, 88, unitType("corvus"), size, 90)

    ifThen(data.summon().greaterThan(12), () => {
        draw.color(64, 192, 64, 255)
        draw.stroke(32)
        draw.line(88, 88, 0, 88)
