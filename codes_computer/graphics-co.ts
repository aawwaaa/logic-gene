...
import { Condition, SingleResult, Value, Var, always, build, control, draw, drawflush, end, getlink, jumpToAfter, operation, print, printflush, prop, read, sensor, set, time, variable, wait, write } from "../generator/core";
...
function waitForToggle(clock: Var<import("../generator/core").Building>) {
...
const running = sensor(clock, prop("enabled")).get("running")

ifNot(running.notEqual(0), () => {
    end()
})
...
switchCounterJump(command, [() => {
    ...
    switchCounterJump(data, [() => {
        draw.clear(value3, value2, value)
    }, () => {
        draw.color(value4, value3, value2, value)
    }, () => {
        pop().to(value)
        draw.stroke(value)
    }, () => {
        draw.line(value4, value3, value2, value)
    }, () => {
        draw.rect(value4, value3, value2, value)
    }, () => {
        const value5 = pop().get()
        draw.poly(value5, value4, value3, value2, value)
    }, () => {
        draw.lineRect(value4, value3, value2, value)
    }, () => {
        const value5 = pop().get()
        draw.linePoly(value5, value5, value3, value2, value)
    }, () => {
        const value5 = pop().get()
        const value6 = pop().get()
        draw.triangle(value6, value5, value4, value3, value2, value)
    }, () => {
